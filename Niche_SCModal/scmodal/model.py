...
class Model(object):
    def __init__(self, batch_size=128, training_steps=10000, seed=1234, n_latent=20,
                 lambdaAE = 10.0, lambdaLA = 10.0, lambdaMNN = 1.0, lambdaGeo = 10.0, lambdaGAN = 1.0, n_KNN = 30,
                 model_path="models", data_path="data", result_path="results", device = "cuda"):
    ...
    def preprocess(self,
        ...
        print("preprocess called with alpha:", alpha, "and eta:", eta) 
        self.adata_A = adata_A_input.copy()
        self.adata_B = adata_B_input.copy()

        self.shared_gene_num = shared_gene_num
        if hasattr(self.adata_A.X, 'toarray'):
            self.emb_A = self.adata_A.X.toarray()
        else:
            self.emb_A = self.adata_A.X

        if hasattr(self.adata_B.X, 'toarray'):
            self.emb_B = self.adata_B.X.toarray()
        else:
            self.emb_B = self.adata_B.X
        print(type(self.emb_A), type(self.emb_B))
        
            # Handle alpha and eta
        if alpha is not None:
            if isinstance(alpha, pd.DataFrame):
                alpha = alpha.loc[self.adata_A.obs_names].values
            assert alpha.shape[0] == self.emb_A.shape[0]
            self.alpha = alpha.astype(np.float32)  # ← keep as NumPy
        else:
            self.alpha = None

        if eta is not None:
            if isinstance(eta, pd.DataFrame):
                eta = eta.loc[self.adata_A.obs_names].values
            assert eta.shape[0] == self.emb_A.shape[0]
            self.eta = eta.astype(np.float32)  # ← keep as NumPy
        else:
            self.eta = None
    
    def update_embeddings(self, alpha=None, eta=None):
        """Update alpha/eta without touching learned weights."""
        if alpha is not None:
            if isinstance(alpha, pd.DataFrame):
                alpha = alpha.loc[self.adata_A.obs_names].values
            assert alpha.shape[0] == self.emb_A.shape[0]
        ...
        if eta is not None:
            ...
            self.eta = None
        print("✅ Updated alpha and eta embeddings in the model (weights preserved).")
    ...
    def train(self, load_ckpt=False):
        begin_time = time.time()
        print("Begining time: ", time.asctime(time.localtime(begin_time)))
        d_alpha = self.alpha.shape[1] if self.alpha is not None else None
        d_eta = self.eta.shape[1] if self.eta is not None else None
        self.E_A = encoder(self.emb_A.shape[1], self.n_latent, d_alpha, d_eta).to(self.device)
        self.E_B = encoder(self.emb_B.shape[1], self.n_latent, None, None).to(self.device)
        ...
        if load_ckpt and os.path.exists(os.path.join(self.model_path, "ckpt.pth")):
            state = torch.load(os.path.join(self.model_path, "ckpt.pth"))

            def safe_load(module, state_dict):
            ...
            print("🧩 Loading checkpoint with shape-safe mapping...")
            safe_load(self.E_A, state['E_A'])
            safe_load(self.E_B, state['E_B'])
            safe_load(self.G_A, state['G_A'])
            safe_load(self.G_B, state['G_B'])
            safe_load(self.D_Z, state['D_Z'])
            # --- Load optimizer states safely ---
            if 'optimizer_G' in state:
                try:
                    optimizer_G.load_state_dict(state['optimizer_G'])
                    print("✅ Loaded optimizer_G state.")
                except Exception as e:
                    print(f"⚠️ Skipping optimizer_G load due to mismatch: {e}")
            if 'optimizer_D' in state:
                try:
                    optimizer_D.load_state_dict(state['optimizer_D'])
                    print("✅ Loaded optimizer_D state.")
                except Exception as e:
                    print(f"⚠️ Skipping optimizer_D load due to mismatch: {e}")
        ...
        for step in range(self.training_steps):
            ...
            #changing to to_array() fix later if needed
            x_A = torch.from_numpy(self.emb_A[index_A, :]).float().to(self.device)
            x_B = torch.from_numpy(self.emb_B[index_B, :]).float().to(self.device)
            alpha_A = torch.from_numpy(self.alpha[index_A, :]).float().to(self.device) if self.alpha is not None else None
            eta_A   = torch.from_numpy(self.eta[index_A, :]).float().to(self.device)   if self.eta is not None else None
            z_A = self.E_A(x_A, alpha_A, eta_A)
            z_B = self.E_B(x_B, None, None)
            x_AtoB = self.G_B(z_A)
            x_BtoA = self.G_A(z_B)
    ...
    def eval(self):
        ...
        alpha_A = torch.from_numpy(self.alpha).float().to(self.device) if self.alpha is not None else None
        eta_A   = torch.from_numpy(self.eta).float().to(self.device)   if self.eta is not None else None
    ...
    def integrate_datasets_links(self, # Use this function for N >= 3 datasets when provided features links for MNN
        ...
        for i in range(num_datasets):
            d_alpha = self.alpha.shape[1] if self.alpha is not None else None
            d_eta = self.eta.shape[1] if self.eta is not None else None
            self.E_dict[i] = encoder(input_feats[i].shape[1], self.n_latent, d_alpha, d_eta).to(self.device)
        ...
        for step in range(self.training_steps):
            ...
            for i in range(num_datasets):
                ...
                z_dict[i] = self.E_dict[i](x_dict[i], self.alpha if self.alpha is not None else None, self.eta if self.eta is not None else None)
    ...
    def integrate_datasets_feats(self, # Use this function for N >= 3 datasets when provided linked features for MNN
        ...
        for i in range(num_datasets):
            self.E_dict[i].train()
            z_dict[i] = self.E_dict[i](torch.from_numpy(input_feats[i]).float().to(self.device), self.alpha if self.alpha is not None else None, self.eta if self.eta is not None else None)
